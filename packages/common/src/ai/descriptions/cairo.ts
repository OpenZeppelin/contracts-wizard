export const cairoPrompts = {
    ...
    Multisig: 'Make a multi-signature smart contract, requiring a quorum of registered signers to approve and collectively execute transactions.',
...
export const cairoRoyaltyInfoDescriptions = {
    ...
    defaultRoyaltyFraction: 'The royalty fraction that will be default for all tokens. It will be used for a token if there\'s no custom royalty fraction set for it.',
...
export const cairoGovernorDescriptions = {
    ...
    votes: 'The type of voting to use. Either erc20votes, meaning voting power with a votes-enabled ERC20 token. Either erc721votes, meaning voting power with a votes-enabled ERC721 token. Voters can entrust their voting power to a delegate.',
    clockMode: 'The clock mode used by the voting token. For now, only timestamp mode where the token uses voting durations expressed as timestamps is supported. For Governor, this must be chosen to match what the ERC20 or ERC721 voting token uses.',
...
export const cairoVestingDescriptions = {
    ...
    schedule: 'A vesting schedule implementation, tokens can either be vested gradually following a linear curve or with custom vesting schedule that requires the implementation of the VestingSchedule trait.',
};
