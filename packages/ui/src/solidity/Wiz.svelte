...
<div class="absolute bottom-8 right-8 h-[calc(100%-188px)]">
  <div class={`${showing ? '' : 'hidden'} ${expanded ? 'w-[40rem]' : 'w-80'} max-w-[400px] min-h-[200px] absolute flex flex-col-reverse right-0 bottom-[4rem] border-0 shadow-xl bg-gray-50 rounded-md animate-fade-up max-h-full overflow-y-auto z-20`}>
  ...
  {#if showing}
    <button class="z-20 absolute flex right-0 bottom-0 items-center justify-center border-0 shadow-xl bg-gray-50 h-12 w-12 p-4 rounded-full cursor-pointer hover:bg-white" on:click={() => {
      showing = false
    }}>
  ...
  {:else}
    <button class="z-20 absolute flex right-0 bottom-0 items-center justify-center border-0 shadow-xl bg-gray-50 h-12 w-36 p-4 rounded-full cursor-pointer hover:bg-white" on:click={() => {
      showing = true
    }}>
      <div class="animate-fade-in">
