...
> Snapshot 1
    ...
    Options:␊
      ...
      --minBurnAmount <string>              An optional minimum amount of tokens that must be burned at once, in whole tokens. Requires the token to be burnable by its holders. Privileged accounts can update the minimum after deployment.␊
      --pausable                            Whether privileged accounts will be able to pause minting, burning and metadata updates, and also transfers of the asset when an allowlist or blocklist is active. Unrestricted transfers are never checked against the faucet. Useful for emergency response.␊
