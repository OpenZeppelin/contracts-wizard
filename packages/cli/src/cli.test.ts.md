...
> Snapshot 1
    ...
    ␊
    Options:␊
      --decimals <string>                   The number of decimals used to represent token amounts. Defaults to 6. Incompatible with the wrappable extension, which uses the decimals of the underlying token.␊
      --premint <string>                    The number of tokens to premint for the deployer.␊
      --wrappable                           Whether to allow wrapping an ERC20 token into a confidential fungible token. Uses the decimals of the underlying token, so it cannot be used with custom decimals.␊
