...
> Snapshot 1
    ...
    ␊
    Options:␊
      --decimals <string>                   The number of decimals used to represent token amounts. Defaults to 6. Incompatible with wrappable, which uses the decimals of the underlying token.␊
      ...
      --wrappable                           Whether to allow wrapping an ERC20 token into a confidential fungible token. Uses the decimals of the underlying token, so it cannot be used with custom decimals. Cannot be used with premint, since preminted tokens would not be backed by the underlying token.␊
