...
export function printContract(contract: Contract, options: { isAccount: boolean }): string {
  const contractAttribute = options.isAccount ? '#[starknet::contract(account)]' : '#[starknet::contract]'
  return formatLines(
    ...spaceBetween(
      [
