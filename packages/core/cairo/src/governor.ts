...
function addBase(c: ContractBuilder, _: GovernorOptions) {
  ...
  c.addUseClause(
    "openzeppelin::governance::governor::GovernorComponent",
    "InternalTrait",
...
function addSettings(c: ContractBuilder, allOpts: Required<GovernorOptions>) {
  ...
  if (allOpts.settings) {
    c.addUseClause(
      `${extensionPath}::GovernorSettingsComponent`,
      "InternalTrait",
...
function getProposalThreshold({
  ...
  decimals,
  votes,
}: Required<GovernorOptions>): { value: string; comment?: string } {
...
function addSettingsLocalImpl(
  c: ContractBuilder,
  _: Required<GovernorOptions>,
) {
  const settingsTrait = {
...
function addQuorumAndVotes(
  c: ContractBuilder,
  allOpts: Required<GovernorOptions>,
) {
...
function addVotesQuorumFractionComponent(
  c: ContractBuilder,
  ...
  c.addUseClause(
    `${extensionPath}::GovernorVotesQuorumFractionComponent`,
    "InternalTrait",
...
function addVotesComponent(c: ContractBuilder, _: Required<GovernorOptions>) {
  ...
  c.addComponent(
    components.GovernorVotesComponent,
    [{ lit: "votes_token" }],
    true,
  );
}

function addQuorumLocalImpl(
  c: ContractBuilder,
  quorum: string,
  comment: string,
) {
...
function addExecution(
  ...
  if (timelock === false) {
    ...
    c.addUseClause(
      `${extensionPath}::GovernorTimelockExecutionComponent`,
      "InternalTrait",
      { alias: "GovernorTimelockExecutionInternalTrait" },
    );
    c.addComponent(
      components.GovernorTimelockExecutionComponent,
