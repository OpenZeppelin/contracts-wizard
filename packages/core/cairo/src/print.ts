...
export function printContract(contract: Contract): string {
  const contractAttribute = contract.account
    ...
    (
      ...
      if (useClause.groupable) {
        (result[useClause.containerPath] =
          result[useClause.containerPath] || []).push(useClause);
      } else {
        (result[STANDALONE_IMPORTS_GROUP] =
          result[STANDALONE_IMPORTS_GROUP] || []).push(useClause);
      }
      return result;
    },
    {},
...
function getLinesFromUseClausesGroup(
  ...
  if (groupName === STANDALONE_IMPORTS_GROUP) {
    ...
    if (group.length == 1) {
      ...
      const names = group
        .map((useClause) => nameWithAlias(useClause))
        .join(", ");
...
function nameWithAlias(useClause: UseClause): string {
  return useClause.alias
    ? `${useClause.name} as ${useClause.alias}`
    : useClause.name;
}
...
function printFunction(fn: ContractFunction): Lines[] {
  ...
  for (let i = 0; i < codeLines.length; i++) {
    ...
    const shouldEndWithSemicolon =
      i < codeLines.length - 1 || fn.returns === undefined;
    if (line !== undefined && line.length > 0) {
      if (
        shouldEndWithSemicolon &&
        !["{", "}", ";"].includes(line.charAt(line.length - 1))
      ) {
        codeLines[i] += ";";
...
function hasInitializer(parent: Component): boolean {
  return (
    parent.initializer !== undefined && parent.substorage?.name !== undefined
  );
}
