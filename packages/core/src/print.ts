...
export function printContract(contract: Contract, opts?: Options): string {
    ...
    return formatLines(
        ...spaceBetween(
            ...
            contract.imports.map(
                (p) => `import "${helpers.transformImport(p)}";`
            ),
            ...
            [
                ...
                [
                    ...
                    ...printInheritance(contract, helpers),
                    "{",
                ].join(" "),
...
function printInheritance(
    ...
    if (contract.parents.length > 0) {
        return [
            "is " +
                contract.parents
                    .map((p) => transformName(p.contract.name))
                    .join(", "),
        ];
    } else {
        return [];
...
function printUsingFor(
    ...
    return contract.using.map(
        (u) =>
            `using ${transformName(u.library.name)} for ${transformName(
                u.usingFor
            )};`
    );
}
...
function printConstructor(contract: Contract, helpers: Helpers): Lines[] {
    ...
    if (
        ...
        const modifiers = helpers.upgradeable
            ? ["initializer public"]
            : parents;
        const args = contract.constructorArgs.map((a) =>
            printArgument(a, helpers)
        );
        const body = helpers.upgradeable
            ...
            : contract.constructorCode;
        const head = helpers.upgradeable
            ? "function initialize"
            : "constructor";
...
function printArgument(
    ...
    return arg.memoryType
        ? [type, arg.memoryType, arg.name].join(" ")
        : [type, arg.name].join(" ");
}
