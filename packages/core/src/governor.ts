...
export function buildGovernor(opts: GovernorOptions): Contract {
  ...
  addBase(c, opts);
  setVotingTimeParameters(c, opts);
  addCounting(c, opts);
  addVotes(c, opts);
  addQuorum(c, opts);
  addTimelock(c, opts);
...
function addBase(c: ContractBuilder, { name }: GovernorOptions) {
  ...
  c.addOverride('Governor', functions.getVotes);
...
function setVotingTimeParameters(c: ContractBuilder, opts: GovernorOptions) {
...
function addBravo(c: ContractBuilder, { bravo }: GovernorOptions) {
  if (bravo) {
    c.addParent({
