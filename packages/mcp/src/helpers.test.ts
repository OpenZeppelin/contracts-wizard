 ...
 */
export async function assertAPIEquivalence<T>(t: ExecutionContext<Context>, params: T, wizardApiFunction: (params: T) => string, expectError?: boolean) {
    ...
    if (expectError) {
        ...
        t.true(mcpResult.includes(apiError.message), `Expected MCP error message to include "${apiError.message}", but got "${mcpResult}"`);
